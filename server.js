...
const GEMINI_API_KEY = process.env.GEMINI_API_KEY; // for the Ask (AI chat) feature — free tier, no card

if (!SERPER_API_KEY) {
  console.warn('⚠️  SERPER_API_KEY is not set. Search requests will fail until you set it.');
}
if (!GEMINI_API_KEY) {
  console.warn('⚠️  GEMINI_API_KEY is not set. The Ask (AI chat) feature will fail until you set it.');
}
...
function geminiChat(messages) {
  return new Promise((resolve, reject) => {
    // Gemini expects a "contents" array with role user/model (not
    // "assistant") and parts: [{ text }]. Convert our simpler shape.
    const trimmed = messages.slice(-20).map((m) => ({
      role: m.role === 'assistant' ? 'model' : 'user',
      parts: [{ text: m.content }],
    }));

    const payload = JSON.stringify({ contents: trimmed });
...
const server = http.createServer(async (req, res) => {
  ...
  if (parsed.pathname === '/api/chat' && req.method === 'POST') {
    try {
      ...
      const raw = await geminiChat(messages);
      const reply = (raw.candidates?.[0]?.content?.parts || [])
        .map((part) => part.text || '')
        .join('\n')
        .trim() || '(no response)';
